$(document).ready(function () {    // Čeka se trenutak kada je DOM(Document Object Model) učitan da bi JS mogao sa njim da manipuliše.
    ...
    $.ajax({
        type: "POST",                                              
        url: "http://localhost:8080/api/clan/prikazTermina",                 
        dataType: "json",  
        ...
        success: function (res) {                                   
            ...
            for (i = 0; i < res.length; i++) {                      
                ...
                let btn1 = "<button id = \"prijava\" class='button1' data-id=" + res[i].id + ">Prijavi se</button>"
...
$(document).on('click', '#prijava', function () {            // kada je button (čija je klasa class = btnSeeMore) kliknut
    ...
    $.ajax({
        type: "POST",                                              
        url: "http://localhost:8080/api/clan/prijavaTermina",                 
        dataType: "json",
        contentType: "application/json",   
        data: JSON.stringify(prijavljenTermin),   
